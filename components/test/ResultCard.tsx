...
function RadarSVG({ radar }: { radar: PersonalityResult["radar"] }) {
  ...
  const polyPoints = values.map((v, i) => {
    ...
    return `${cx + Math.cos(toRad(i)) * r * ratio},${cy + Math.sin(toRad(i)) * r * ratio}`;
  }).join(" ");

  return (
    <svg viewBox="0 0 200 200" className="w-full max-w-[220px] mx-auto">
      {[0.33, 0.66, 1].map((ratio, j) => (
        <polygon key={j} points={labels.map((_, i) =>
          `${cx + Math.cos(toRad(i)) * r * ratio},${cy + Math.sin(toRad(i)) * r * ratio}`).join(" ")}
          fill="none" stroke="rgba(255,179,71,0.2)" strokeWidth="1" />
      ))}
      {labels.map((_, i) => (
        <line key={i} x1={cx} y1={cy} x2={cx + Math.cos(toRad(i)) * r} y2={cy + Math.sin(toRad(i)) * r}
          stroke="rgba(255,179,71,0.15)" strokeWidth="1" />
      ))}
      <polygon points={polyPoints} fill="rgba(255,179,71,0.25)" stroke="#FFB347" strokeWidth="2" strokeLinejoin="round" />
      {values.map((v, i) => {
      ...
      {labels.map((l, i) => (
        <text key={i} x={cx + Math.cos(toRad(i)) * (r + 22)} y={cy + Math.sin(toRad(i)) * (r + 22)}
          textAnchor="middle" dominantBaseline="middle" className="text-[10px] fill-dark/60 font-bold">{l.label}</text>
      ))}
    </svg>
  );
}
...
export function ResultCard({ result, onRetry }: ResultCardProps) {
  ...
  const handleScreenshot = async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(cardRef.current, { backgroundColor: "#fff8f0", scale: 2, useCORS: true });
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
      ...
      {/* 截图区域 */}
      <div ref={cardRef} className="glass-heavy p-6 sm:p-8 relative overflow-hidden"
        style={{ borderTop: `4px solid ${result.color}` }}>

        <div className="text-center mb-6">
          <motion.p className="text-5xl mb-3" initial={{ scale: 0 }} animate={{ scale: 1 }}
            transition={{ type: "spring", delay: 0.3 }}>{result.title.slice(0, 2)}</motion.p>
          <motion.h2 className="text-2xl sm:text-3xl font-extrabold gradient-text mb-1"
            initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>{result.title}</motion.h2>
          <motion.p className="text-sm text-dark/40" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>{result.subtitle}</motion.p>
        </div>

        <motion.div className="flex flex-wrap justify-center gap-2 mb-6">
          {result.tags.map(tag => (
            <span key={tag} className="px-3 py-1 rounded-full text-xs font-bold text-white"
              style={{ background: result.color }}>{tag}</span>
          ))}
        </motion.div>
        ...
        <motion.div className="mb-6">
          <p className="text-sm font-bold text-dark/60 mb-2 text-center">📡 社交雷达图</p>
      ...
      <div className="flex gap-3 mt-6">
        <button onClick={onRetry}
          className="flex-1 flex items-center justify-center gap-2 px-4 py-3 glass text-dark font-bold
            rounded-xl hover:bg-primary/5 transition-all text-sm">
        ...
        <button onClick={handleScreenshot} disabled={saving}
          className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-primary
            to-accent text-white font-bold rounded-xl shadow-lg shadow-accent/20 hover:scale-105
            transition-all text-sm disabled:opacity-60">
          <Share2 size={16} />{saving ? "生成中..." : "保存海报"}
        </button>
      </div>
    </motion.div>
  );
